...
public class App{
    ...
    public static void main(String[] args) {
        ...
        check("Add another new item", cart.getItemCount() == 2 && cart.getTotalPrice() == 55.0); // 30+25

        cart.addItem("P002", 1); // Soda x5+1=6
        check("Add item to meet bulk discount threshold", cart.getItemCount() == 2 && cart.getTotalPrice() == 57.0); // 30+27
        ...
        check("BOGO Strategy (1 item)", pricingService.calculateDiscountedPrice(singleApple) == 10.0);
        check("BOGO Strategy (2 items)", pricingService.calculateDiscountedPrice(twoApples) == 10.0);
        check("Bulk Strategy (below threshold)", pricingService.calculateDiscountedPrice(fiveSodas) == 25.0);
        check("Bulk Strategy (above threshold)", pricingService.calculateDiscountedPrice(tenSodas) == 45.0);
        check("Default Strategy", pricingService.calculateDiscountedPrice(normalBread) == 60.0);
        ...
        check("Promotion update works", pricingService.calculateDiscountedPrice(threeApples) == 15.0); // 30 * 0.5 = 15.0
